...
```sh
curl -sSfL https://raw.githubusercontent.com/DioneProtocol/odyssey-network-runner/develop/scripts/install.sh | sh -s
```

To install a specific version, just append the desired version to the command (must be an existing github tag like v0.0.1)

```sh
curl -sSfL https://raw.githubusercontent.com/DioneProtocol/odyssey-network-runner/main/scripts/install.sh | sh -s v0.0.1
```
...
This is only needed by advanced users who want to modify or test Odyssey Network Runner in specific ways.
...
```sh
git clone https://github.com/DioneProtocol/odyssey-network-runner.git
```
...
The E2E test checks `odyssey-network-runner` RPC communication and control. It starts a network against a fresh RPC
...
## Using `odyssey-network-runner`

You can import this repository as a library in your Go program, but we recommend running `odyssey-network-runner` as a binary. This creates an RPC server that you can send requests to in order to start a network, add nodes to the network, remove nodes from the network, restart nodes, etc.. You can make requests through the `odyssey-network-runner` command or by making API calls. Requests are "translated" into gRPC and sent to the server.

**Why does `odyssey-network-runner` need an RPC server?** `odyssey-network-runner` needs to provide complex workflows such as replacing nodes, restarting nodes, injecting fail points, etc.. The RPC server exposes basic operations to enable a separation of concerns such that one team develops a test framework, and the other writes test cases and controlling logic.
...
# or
odyssey-network-runner ping \
--log-level debug \
--endpoint="0.0.0.0:8080"
```

To start a new Odyssey network with five nodes (a cluster):

```bash
# replace execPath with the path to OdysseyGo on your machine
# e.g., ${HOME}/go/src/github.com/DioneProtocol/odysseygo/build/odysseygo
ODYSSEYGO_EXEC_PATH="odysseygo"

curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${ODYSSEYGO_EXEC_PATH}'","numNodes":5,"logLevel":"INFO"}'

# or
odyssey-network-runner control start \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--number-of-nodes=5 \
--odysseygo-path ${ODYSSEYGO_EXEC_PATH}
```

Additional optional parameters which can be passed to the start command:

```bash
--plugin-dir ${ODYSSEYGO_PLUGIN_PATH} \
...
For example, to set `odysseygo --http-host` flag for all nodes:

```bash
# to expose local RPC server to all traffic
# (e.g., run network runner within cloud instance)
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${ODYSSEYGO_EXEC_PATH}'","globalNodeConfig":"{\"http-host\":\"0.0.0.0\"}"}'
...
The network-runner supports odysseygo node configuration at different levels.
...
2. `--global-node-config` is a JSON string representing a _single_ odysseygo config, which will be applied to **all nodes**. This makes it easy to define common properties to all nodes. Whatever is set here will be _combined_ with the standard set above.
...
'{"execPath":"'${ODYSSEYGO_EXEC_PATH}'","customNodeConfigs":
{
...
# or
odyssey-network-runner control health \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
# or
odyssey-network-runner control uris \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
# or
odyssey-network-runner control status \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
```bash
odyssey-network-runner control \
--request-timeout=3m \
stream-status \
--push-interval=5s \
--log-level debug \
...
# or
odyssey-network-runner control save-snapshot snapshotName
```
...
# or
odyssey-network-runner control load-snapshot snapshotName
```

An odysseygo binary path and/or plugin dir can be specified when loading the snapshot. This is
optional. If not specified, will use the paths saved with the snapshot:

```bash
curl -X POST -k http://localhost:8081/v1/control/loadsnapshot -d '{"snapshot_name":"node5","execPath":"'${ODYSSEYGO_EXEC_PATH}'","pluginDir":"'${ODYSSEYGO_PLUGIN_PATH}'"}'

# or
odyssey-network-runner control load-snapshot snapshotName --odysseygo-path ${ODYSSEYGO_EXEC_PATH} --plugin-dir ${ODYSSEYGO_PLUGIN_PATH}
```
...
# or
odyssey-network-runner control get-snapshot-names
```
...
# or
odyssey-network-runner control remove-snapshot snapshotName
```
...
# or
odyssey-network-runner control create-subnets '[{}]'
```
...
# or
odyssey-network-runner control create-subnets '[{"participants": ["node1", "node2"]}]'
```
...
# or
odyssey-network-runner control create-subnets '[{"participants": ["node1", "node2", "testNode"]}]'
...
# or
odyssey-network-runner control create-subnets '[{}, {"participants": ["node1", "node2", "node3"]}, {"participants": ["node1", "node2", "testNode"]}]'
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_CONTENTS'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_id": "'$SUBNET_ID'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_id": "'$SUBNET_ID'", "chain_config": "'$CHAIN_CONFIG_PATH'", "network_upgrade": "'$NETWORK_UPGRADE_PATH'", "subnet_config": "'$SUBNET_CONFIG_PATH'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_spec": "{"participants": ["node1", "node2", "testNode"]}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_id": "'$SUBNET_ID'", "per_node_chain_config": "'$PER_NODE_CHAIN_CONFIG'", "network_upgrade": "'$NETWORK_UPGRADE_PATH'", "subnet_config": "'$SUBNET_CONFIG_PATH'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
odyssey-network-runner control remove-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
node5
...
curl -X POST -k http://localhost:8081/v1/control/restartnode -d '{"name":"node1","execPath":"'${ODYSSEYGO_EXEC_PATH}'","logLevel":"INFO"}'

# or
odyssey-network-runner control restart-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--odysseygo-path ${ODYSSEYGO_EXEC_PATH} \
...
# a different execPath
curl -X POST -k http://localhost:8081/v1/control/addnode -d '{"name":"node99","execPath":"'${ODYSSEYGO_EXEC_PATH}'","logLevel":"INFO"}'

# or
odyssey-network-runner control add-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--odysseygo-path ${ODYSSEYGO_EXEC_PATH} \
...
# or
odyssey-network-runner control pause-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
node99
...
# or
odyssey-network-runner control resume-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
node99
...
`--node-config` allows to specify specific odysseygo config parameters to the new node.
...
OdysseyGo exposes a "test peer", which you can attach to a node.
(See [here](https://github.com/DioneProtocol/odysseygo/blob/develop/network/peer/test_peer.go) for more information.)
...
# or
odyssey-network-runner control attach-peer \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1
...
# or
odyssey-network-runner control send-outbound-message \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1 \
...
# or
odyssey-network-runner control stop \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
```bash
# download from https://github.com/DioneProtocol/odysseygo/releases
# or build
rm -rf ${HOME}/go/src/github.com/DioneProtocol/odysseygo/build
cd ${HOME}/go/src/github.com/DioneProtocol/odysseygo
./scripts/build.sh

cd ${HOME}/go/src/github.com/DioneProtocol/subnet-evm
go build -v \
-o ${HOME}/go/src/github.com/DioneProtocol/odysseygo/build/plugins/srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy \
./plugin

# make sure binaries are built
find ${HOME}/go/src/github.com/DioneProtocol/odysseygo/build
# for example
# .../build
# .../build/plugins
...
```bash
# replace execPath with the path to OdysseyGo on your machine
ODYSSEYGO_EXEC_PATH="${HOME}/go/src/github.com/DioneProtocol/odysseygo/build/odysseygo"
ODYSSEYGO_PLUGIN_PATH="${HOME}/go/src/github.com/DioneProtocol/odysseygo/build/plugins"

curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${ODYSSEYGO_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${ODYSSEYGO_PLUGIN_PATH}'","blockchainSpecs":[{"vm_name":"subnetevm","genesis":"/tmp/subnet-evm.genesis.json"}]}'
...
You can create a custom OdysseyGo genesis with function `network.NewOdysseyGoGenesis`:
...
The network runner allows users to interact with an OdysseyGo network using the `network.Network` interface:
...
```go
// Node represents an OdysseyGo node
type Node interface {
  // Return this node's name, which is unique
  // across all the nodes in its network.
  GetName() string
  // Return this node's Odyssey node ID.
  ...
  // Return this node's odysseygo binary path
